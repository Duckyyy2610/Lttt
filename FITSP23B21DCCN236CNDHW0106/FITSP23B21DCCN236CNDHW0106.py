...
if __name__ == '__main__':
    ...
    for i in range(1, max_digit+2):
        bin_poly.extend(binary_generate(i))
    # print(test_2)
    poly_transform = set()
    for testcase in bin_poly:
        poly_transform.add(Poly(testcase, x, domain='GF(2)').as_expr())
    with open('output/FITSP23B21DCCN236CNDHW0106(output 4).txt', 'w') as file:
        ...
        for i in range(len(test_4)):
            poly_root = x**(int(test_4[i])) + 1
            file.write(f'\nTest case {i+1}:')
            file.write(f'\n\n \tInput: Da thu {poly_root}\n\tOutput: \n\t isReverse = 0')
