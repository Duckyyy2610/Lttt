...
if __name__ == '__main__':
    ...
    bit_repr = []
    test_1 = []
    with open('FITSP23B21DCCN236CNDHW0106(input).txt', 'r') as file:
        file = file.readlines()
        file.pop(0)
        for testcase in file:
            digit = ''
            if 'Test case' in testcase:
    ...
    # print(len(test))
    with open('FITSP23B21DCCN236CNDHW0106(output).txt', 'w') as file:
    ...
    with open('FITSP23B21DCCN236CNDHW0106(input).txt', 'a') as file:
        for i in range(len(test_2)):
            file.write(f'Test case {i + 1} : {test_2[i]}\n')

    with open('FITSP23B21DCCN236CNDHW0106(output).txt', 'a') as file:
        file.write('\n2) Kiem tra mot da thuc cho truoc co phai toi gian hay khong\n')
        for i in range(len(test_2)):
            file.write(f'\nTest case {i + 1}:')
    ...
    with open('FITSP23B21DCCN236CNDHW0106(output).txt', 'a') as file:
        file.write('\n3) Phan tich x^n + 1 thanh cac nhan tu toi gian \n')
        for i in range(len(test_1)):
            # print(test_1[i])
            file.write(f'\nTest case {i + 1}:')
            file.write(f'\n\n \tInput: x^{test_1[i]} + 1\n\tOutput: {factorize_polynomial(int(test_1[i]))}\n')
    ...
    bin_poly = set()
    for i in range(len(test_1)):
        max_digit = max(max_digit, int(test_1[i]))
    ...
    with open('FITSP23B21DCCN236CNDHW0106(output).txt', 'a') as file:
        file.write('\n4) Liet ke tat ca cac nhan tu uoc so phan biet cua x^n + 1')
        for i in range(len(test_1)):
            poly_root = Poly(x**(int(test_1[i])) + 1,x)
            file.write(f'\nTest case {i+1}:')
